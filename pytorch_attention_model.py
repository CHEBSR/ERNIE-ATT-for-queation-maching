...
# 设置参数及文件路径
os.environ['CUDA_VISIBLE_DEVICES'] = '1'  # 程序可调用的GPU的ID
max_seq_length = 60  # 输入文本最大长度
learning_rate = 2e-5  # 模型学习率
num_epochs = 9  # 训练最大迭代次数
batch_size = 100  # 训练时每个batch中的样本数
patience = 2  # 早停轮数
file_name = 'baseline'  # 指定输出文件的名字
# model_name_or_path = './pretrain_models/ernie_grame/'  # 预训练模型权重载入路径
model_name_or_path = "peterchou/ernie-gram"
# model_name_or_path = "nghuyong/ernie-3.0-base-zh"
# model = AutoModel.from_pretrained("peterchou/ernie-gram")
train_input = './data/pytorch_model/train/'  # 完成预处理的训练集载入路径
test_input = './data/pytorch_model/test/'  # 完成预处理的测试集载入路径
# model_save_dir = f'model_save/ernie-3.0-base-zh/'
...
def read_data(file_name):
    ...
    with open(file_name, 'r', encoding='utf-8') as f:
        for line in f.readlines():
            ...
            if line:
                line = line.split('\t')
                examples.append(InputExample(s1=line[0], s2=line[1], label=int(
                    line[2]) if len(line) == 3 else None))
    return examples
...
#             model_name_or_path, num_labels=num_class)
...
#         # self.bert = AutoModelForMaskedLM.from_pretrained(model_name_or_path, config=self.config)
...
#         all_hidden_states = output.hidden_states  # for all layers
...
def prob_postprocess(y_pred, p0=0.6903327690476333):
    # print('y_pred', y_pred[:5])
    # prior = np.array([0.6903327690476333, 0.3096672309523667])  # 训练集 oppo正负样本比例
    prior = np.array([p0, 1 - p0])
    # Counter({0: 377002, 1: 282716})
    # prior = np.array([0.5714, 0.42854]) # 4data-no-paws
    ...
    if post:
        post = np.array(post)
        print(y_pred.shape, y_pred_uncertainty.shape, post.shape)
...
def postprocess(data, pred, debug=False):
    post = []
    for line, lable in tqdm(zip(data, pred)):
        ...
        if r2:
            post.append(1)
            if debug:
                print(f'label changed: {line.s1} : {line.s2}, origin: {lable}')
        else:
            post.append(lable)
    post = np.array(post)
...
if __name__ == '__main__':
    ...
    for fold, (train_index, valid_index) in enumerate(skf.split(all_label, all_label)):
        ...
        model_path = model_save_dir + 'ernie_' + \
            file_name + '_{}.bin'.format(fold)
        if os.path.exists(model_path):
            logger.info(f"resume model from {model_path}")
        ...
        for epoch in range(num_epochs):
            ...
            for batch in tqdm(train_loader):
                ...
                with autocast():
                    y_pred, loss = model(x_ids, x_mask, x_sids,
                                         y=y_truth, loss_fn=loss_fn)
            ...
            with torch.no_grad():
                for i, batch in tqdm(enumerate(valid_loader)):
                    ...
                    with autocast():
                        y_pred, loss = model(
                            x_ids, x_mask, x_sids, y_truth, loss_fn)
            ...
            if best_f1 < f1:
                logger.info(
                    f"f1 improved from {best_f1} to {f1}, and new acc={acc}, saved")
                early_stop = 0
                best_f1 = f1
                valid_best = valid_preds_fold
                torch.save(model.state_dict(), model_save_dir + 'ernie_' +
                           file_name + '_{}.bin'.format(fold))
        ...
        model.load_state_dict(torch.load(
            model_save_dir + 'ernie_' + file_name + '_{}.bin'.format(fold)))
    ...
    # 保存概率文件
    np.savetxt('./submit/train_prob/train_bert_' +
               file_name + '.txt', oof_train)
    ...
    analysis[analysis['label'] != analysis['pred']].to_csv(
        'analysis_{}.csv'.format(f1), index=False)

    # 后处理
    logger.info(f"oof_test shape: {oof_test.shape}")
    oof_test = prob_postprocess(oof_test)
    logger.info(f"after oof_test shape: {oof_test.shape}")
